...
class AdItem(scrapy.Item):
    ...
    def process_pricing(input):
        ...
        price_large = soup.find("span", class_="type-title-large").get_text()
        price_medium = soup.find("span", class_="type-title-medium").get_text()
        price_small = soup.find("span", class_="type-title-small").get_text()
        category = soup.find("span", class_="olx-badge").get_text()
        if price_large:
            pricing_dict['price_large'] = price_large
        if price_medium:
            pricing_dict['price_medium'] = price_medium
        if price_small:
            pricing_dict['price_small'] = price_small
        if category:
            pricing_dict['category'] = category
    ...
    def process_breadcrumb(input):
        ...
        return " > ".join(item.get_text(strip=True) for item in breadcrumb_items)
    ...
    # Promoted from ad['properties'] (apartment/house ads populate these).
    real_estate_type = scrapy.Field()
    condominio = scrapy.Field()
    size = scrapy.Field()
    rooms = scrapy.Field()
